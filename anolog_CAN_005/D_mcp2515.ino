...
void mcp2515_read_regs (uint8_t addr, uint8_t* buf, uint8_t n, int CS_pin)
{
    int i;
...
void mcp2515_write_regs (uint8_t addr, const uint8_t* buf, uint8_t n, int CS_pin)
{
    int i;
...
static void mcp2515_write_reg (uint8_t addr, uint8_t buf, int CS_pin)
{
...
static void mcp2515_bit_modify (uint8_t addr, uint8_t mask, uint8_t bits, int CS_pin)
{
      digitalWrite (CS_pin, LOW);
 ...
 */
uint8_t mcp2515_get_msg (uint8_t rx_buf, uint32_t *id, uint8_t *data, uint8_t *len, int CS_pin)
{
    uint8_t buf[5];
    uint8_t extended;

    mcp2515_read_regs (REG(RX, rx_buf, SIDH), buf, 5, CS_pin);
    *len = buf[4] & 0x0f;
    if (*len > 8)
        *len = 8;
    mcp2515_read_regs (REG(RX, rx_buf, D0), data, *len,CS_pin);
    ...
    mcp2515_bit_modify (CANINTF, 1 << (RX0IF + rx_buf), 0, CS_pin);
...
void mcp2515_set_rx_mask (uint8_t mask_num, uint32_t mask, uint8_t extended,int CS_pin)
{
    uint8_t reg;
    uint8_t buf[4];
    ...
    mcp2515_write_regs (reg, buf, 4, CS_pin);
}

void mcp2515_set_rx_filter (uint8_t filter_num, uint32_t filter, uint8_t extended, int CS_pin)
{
    uint8_t reg;
    uint8_t buf[4];
