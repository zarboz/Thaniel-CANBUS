...
//********************** Begin ***********************
void CAN_begin(uint32_t bit_period) {
  SPI.begin();
...
void CAN_setMaskOrFilter(byte mask, byte b0, byte b1, byte b2, byte b3) {
  CAN_setMode(CAN_MODE_CONFIG); //  1= Configuration, 2 = NORMAL, 3 = SLEEP, 4 = LISTEN, 5 = LOOPBACK 
  CAN_setRegister(mask, b0);
  CAN_setRegister(mask+1, b1);
  CAN_setRegister(mask+2, b2);
  CAN_setRegister(mask+3, b3);
  CAN_setMode(CAN_MODE_LISTEN_ONLY); // (was 1) 1= Configuration, 2 = NORMAL, 3 = SLEEP, 4 = LISTEN, 5 = LOOPBACK
}
...
void CAN_getMessage ()
{
  ...
  uint8_t extended;
  //  CanMessage m;
  //test = mcp2515_get_msg (0, &id, data, &len);
