...
//  if(IGN_status >0)
  ...
  if ((millis() - time_CAN_in) > 60000) // if time since last IKE can message was > 1 second
  {
    ...
    while(!CAN_available()) //wait here until kbus signals come  Should not get here if power off worked.
    {//just wait.
    }// end if available
    digitalWrite(off_pin,LOW); //low is ON (high turns off)
    Serial.println("Can_bus started");
  }
